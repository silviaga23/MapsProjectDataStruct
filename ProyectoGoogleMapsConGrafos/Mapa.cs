...
namespace ProyectoGoogleMapsConGrafos
{
    public partial class Mapa : Form
    {
        ...
        {
            JsonSeriza Serializador = JsonSeriza.GetInstancia();
            ...
            for (int i = 0; i < objetosResultante.Length; i++)
            {
                ...
                GMarkerGoogle Marcador = new GMarkerGoogle(new PointLatLng(Convert.ToDouble(datos["Latitud"]), Convert.ToDouble(datos["Longitud"])), GMarkerGoogleType.red);
                Marcador.ToolTipMode = MarkerTooltipMode.OnMouseOver;
                Marcador.ToolTip = new GMapRoundedToolTip(Marcador);
                Brush ColorFondoInformacion = new SolidBrush(Color.Gray);
                Font letra = new Font("Arial", 12);
                ColorFondoInformacion.GetType();
                Marcador.ToolTip.Stroke.Color = Color.DarkGray;
        ...
        private void MarcartRutas()
        {
            ...
            for (int i = 0, j = 1; j < gMapControl1.Overlays[1].Markers.Count;i++)
            {
                ...
                RutaObtenida.Stroke.Color = Color.Turquoise;
        ...
        private void TrasarRutaArco(GMapMarker pMarcador)
        {
            ...
            string result = "\n";
            Brush ColorFondoInformacion = new SolidBrush(Color.Turquoise);
            ...
            switch (this.vgContadorIndicadoresDeRuta)
            {
                ...
                case 1:
                    ...
                    Edge<Place> arco = this.Gestor.GetArco(this.vgMarcadorA.Tag.ToString(), this.vgMarcadorB.Tag.ToString());
                    if (arco != null)
                    {
                        List<Place> listaLugares = new List<Place> { arco.GetVertA().Info, arco.GetVertB().Info };
                        ...
                        this.vgCapaMarcadores.Markers.Add(Marcador);
                        this.gMapControl1.Overlays[2] = this.vgCapaMarcadores;
                        result += "Punto A: " + arco.GetVertA().Info.GetName() + "\n";
                        result += "Punto B: " + arco.GetVertB().Info.GetName() + "\n";
                        result += "Peso: " + arco.GetWeight() + " Metros";
        ...
        {
            List<Place> listaLugaresAyacentes = this.Gestor.ObtenerVerticesAyacentes(pMarcador.Tag.ToString());
            string result = "\n";
            if (listaLugaresAyacentes != null)
            {
                ...
                {
                    RutaObtenida = new GMapRoute(new List<PointLatLng> { pMarcador.Position, new PointLatLng { Lat = listaLugaresAyacentes[i].GetLati(), Lng = listaLugaresAyacentes[i].GetLongit() } }, "Camino");
                    RutaObtenida.Stroke.Width = 2;
                    RutaObtenida.Stroke.Color = Color.Red;
                    this.gMapControl1.Overlays[3].Routes.Add(RutaObtenida);
                    result += String.Format("{0}      \t{1}",listaLugaresAyacentes[i].GetName(), RutaObtenida.Distance * 1000 + " M") + "\n";
                }
                ResetMap();
            }
            else
        ...
        {
            GMarkerGoogle Marcador;
            Brush ColorFondoInformacion = new SolidBrush(Color.Black);
            Font f = new Font("Arial", 7, FontStyle.Bold);
            string result = "";
            switch (this.vgContadorIndicadoresDeRuta)
            {
                ...
                case 1:
                    this.vgContadorIndicadoresDeRuta++;
                    this.vgMarcadorB = pMarcador;
                    List<Place> listaCaminoMinimo = this.Gestor.GetRutaMinimaDijkstra(this.vgMarcadorA.Tag.ToString(), this.vgMarcadorB.Tag.ToString());
                    ...
                    this.btnLlegar.BackColor = Color.Crimson;
                    break;
            }
            ResetMap();
        }
        private string  MarcarRuta(List<Place> pListaDatos)
        {
            string result = "", costo = "\n";
            if (pListaDatos != null && pListaDatos.Count > 0)
            {
                ...
                {
                    listaDePuntosLongLati.Add(new PointLatLng(pListaDatos[i].GetLati(), pListaDatos[i].GetLongit()));
                    result +=pListaDatos[i].GetName() + "\n";
                }
        ...
        {
            Place lugarEncontrado = this.Gestor.BuscarVerticePorNombre(pNombreVertice);
            ...
            this.textDatosVerticeBuscado.Text = String.Format("\n\n Nombre: {0}\n Longitud: {1}\n Latitud: {2}\n", lugarEncontrado.GetName(), lugarEncontrado.GetLongit(), lugarEncontrado.GetLati());
            this.vgLatitudInicial = lugarEncontrado.GetLati();
            this.vgLongitudInicial = lugarEncontrado.GetLongit();
        }
        ...
        private void btnLlegar_Click(object sender, EventArgs e)
        {
            ...
            if (!this.vgVerAdyacentes && !this.vgVerArco)
            {
                ...
                this.btnLlegar.BackColor = Color.Aqua;
                MarcartRutas();
            }
            else
            {
        ...
        private void btnVerVerticesAdyacentes_Click(object sender, EventArgs e)
        {
            if (!this.vgVerArco && !this.vgTrazarRuta)
            {
                ...
                this.btnVerVerticesAdyacentes.BackColor = Color.Aqua;
                this.textDatosVerticeBuscado.Text = "";
                this.MarcartRutas();
            }
            else
        ...
        private void btnVerArco_Click(object sender, EventArgs e)
        {
            ...
            if (!this.vgVerAdyacentes && !this.vgTrazarRuta)
            {
                ...
                this.btnVerArco.BackColor = Color.Aqua;
                this.textDatosVerticeBuscado.Text = "";
                this.MarcartRutas();
            }
            else
        ...
        private void LblNombreVertice_Click(object sender, EventArgs e)
        {
        ...
        private void BtnVerVerticesAdyacentes_MouseMove(object sender, MouseEventArgs e)
        {
            this.btnVerVerticesAdyacentes.BackColor = Color.Aqua;
        }
    }
